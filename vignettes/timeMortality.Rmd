...
mortality of insect pests. Figure \@ref(fig:plots) 
is designed to check whether a complementary log-log link 
function can be used to give a fair account of the data.
...
a complementary log-log link function, as against
a logit link, the difference is small.  The difference
does matter, for extrapolation to  mortality values
(commonly 99% or greater) that lie close to or beyond 
the limits of the data. The fitted models should, accordingly,
be treated as providing a best guess.
...
function.  For the data considered here, it is necessary
to model the dispersion factor --- high in the middle of
the range of data, and reducing at the extremes.

An alternative that will be investigated is the use of
binomial errors, with observation level random effects
...
error, as for stats::glm()`.  This will not be explored here, as
it does not offer an equivalent of the `glmmTMB` dispformula.
...
One has to experiment to get these models to fit.  Unfortunately,
the model `HCbb2s.cll` (add a degree 2 normal spline curve to the
model for the fixed effects returns `NaN`s for the variance
parameters.  Some of the possibilities are:
...
in our case `x=TrtTime`) appear to work reasonably well.
* Normal splines, or polynomials, may be fitted for modeling 
the fixed effects, as an alternative to a line. 
...
variation between observations.  By comparison
...
```{r only-obslevel, fig.width=3.75, fig.asp=0.95, bot=-1, top=1.5, out.width="32%",  fig.align='center', fig.show="hold", message=FALSE, echo=FALSE, mar=c(3.1,3.1,2.6,1.1), fig.cap = cap2}

HCBIobs.cll <- suppressWarnings(
...
```
### Compare with fits using `lme4::glmer()`

This is an alternative to `glmmTMB()`, for fits that
assume a binomial error, plus observation level random
effects.  The AIC statistics are not consistent with 
those given using `glmmTMB()`.

```{r glmerFits}
## Comparisons using `glmer()`
HCglmerBIobs.cll <- suppressWarnings(
  glmer(cbind(Dead, Live) ~ 0 + trtGp/scTime + (1|obs) + (1|trtGpRep),
        family=binomial(link='cloglog'), nAGQ=0, data=HawCon)  )
...
closer to that from the `glmmTMB()` fit, but generates a warning
about convergence.  In practical terms, the issue may be that the
criterion for a warning is set overly tight.

```{r cfAICs}
cfAIC <- suppressWarnings(
...
log-log link, should be the same. Instead, they are 716.67
(for the `glmmTMB` model), and 719.48 (for the `glmer` model).
The third and fourth values, both for the logit link, are
720.69 (`glmmTMB`) and 721.54 (`logit`).  In both cases,
`glmer` models that allow for overall curvature give an increased
AIC statistic.
...
```{r DHARMa, fig.width=3.75, fig.asp=0.95, bot=-1, top=1.5, out.width="32%", warning=0, fig.align='center', fig.show="hold", message=FALSE, echo=FALSE, mar=c(3.1,3.1,2.6,1.1), fig.cap=cap3}
set.seed(29)
simRef <- suppressWarnings(
  simulateResiduals(HCbb.cll, n=250, seed=FALSE)  )
plotQQunif(simRef)
...
```{r cap4, echo=FALSE}
cap4 <- "Quantile residuals, by treatment group."
```

```{r residBYgp, echo=FALSE, fig.width=5, fig.asp=0.7, bot=-2.5, warning=FALSE, fig.align='center', fig.show="hold", message=FALSE, warning=FALSE, out.width="49%", echo=FALSE, fig.cap=cap4}
...
### Models with binomial errors and observation level random effects. {-}
...
```{r BIobsCLL, fig.width=3.75, fig.asp=0.95, bot=-1, top=2.5, out.width="49%",  fig.align='center', fig.show="hold", message=FALSE, warning=0, echo=FALSE, mar=c(3.1,3.1,2.6,1.1), fig.cap=cap5, echo=TRUE}
set.seed(29)
...
```{r cap7, echo=FALSE}
cap7 <- "Fitted lines and quadratic curves, for the models where the
...
```{r glmmTMB-altFits-gph, fig.width=9, fig.height=4.5, fig.show='hold', top=2, out.width="100%",  fig.align='center', message=FALSE, warning=0, fig.cap=cap4, echo=FALSE}
library(lattice)
...
# hatLgt2 <- predict(HCbb2s.logit, newdata=subset(dat, link=='logit'), se=TRUE, 
# allow.new.levels=TRUE)
dat <- within(dat, {fit<-c(hatClog$fit,hatLgt$fit); 
# fit2<-c(hatClog2$fit,hatLgt2$fit)
se.fit<-c(hatClog$se.fit,hatLgt$se.fit);
# se.fit2<-c(hatClog2$se.fit,hatLgt2$se.fit)
})
...
This, and the use of `update()` to add the quadratic term in
`TrtTime`, avoids warning messages that are otherwise generated. 
...
data.^[See <https://robjhyndman.com/hyndsight/aic/> for a detailed commentary>]  Thus, 
it is not really legitimate to compare models that use a different
link function.  The model with the lower AIC is the preferred model.
...
mnam <- c('BB: Complementary log-log link', 'BB: logit link', 
          'Obs level REs, cloglog link', 'Obs level REs, logit link')
bbmle::AICtab(HCbb.cll,HCbb.logit, 
              HCBIobs.cll, HCBIobs.logit, mnames=mnam)
```
