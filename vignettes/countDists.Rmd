...
Figure \@ref(fig:cfDBI-BB) compares the binomial distribution 
...
```{r cfDBI-BB, fig.width=9, fig.height=4, echo=FALSE, out.width='95%', fig.show='hold', fig.cap=cap2, eval=noneMiss}
```
...
Figure \@ref(fig:cfDBI-BB) highlighted the extent to which the
...
```{r cfFits, message=FALSE, echo=TRUE, eval=noneMiss}
```
...
Figure \@ref(fig:cfsim) shows diagnostic plots for a 
...
```{r cfsim, out.width='98%', fig.width=7.0, fig.height=2.25, echo=FALSE, fig.show='hold', fig.cap=cap4, eval=noneMiss}
```

Figure \@ref(fig:cfq2) shows diagnostic plots for a 
...
```{r cfq2,  out.width='98%', fig.width=7.0, fig.height=2.25, echo=FALSE, fig.show='hold', fig.cap=cap5, eval=noneMiss}
```

The worm plots give the clearest picture.  Figure \@ref(fig:cfsim)B
makes it clear that the data is not binomial, while Figure
\@ref(fig:cfq2)B indicates that the data are broadly consistent 
with a betabinomial distribution. 
...
```{r cfMales, echo=1:2, eval=noneMiss}
```
...
```{r binFitProbs, echo=1:3, eval=noneMiss}
```
...
Figure \@ref(fig:rqmales) shows worm plots for the binomial
and betabinomial models:

```{r rqmales, out.width='85%', fig.width=8.4, fig.height=3.1, echo=FALSE,  fig.show='hold', fig.cap=cap6, eval=noneMiss}
```
...
```{r cf-AIC-males, echo=FALSE, eval=noneMiss}
```
...
```
The first set of counts are the @rutherford1908electrical polonium radioactive decay counts that give the number of scintillations in 2608 1/8 minute intervals.
```{r ruge, echo=F}
```
...
```{r wormpois, out.width='99%', fig.width=8.0, fig.height=2.5, echo=FALSE, fig.show='hold', fig.cap=cap8}
```
